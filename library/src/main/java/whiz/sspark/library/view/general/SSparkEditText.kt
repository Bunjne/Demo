...
class SSparkEditText : AppCompatEditText {
    ...
    private fun style(attrs: AttributeSet?) {
        val attributes = context.obtainStyledAttributes(attrs, R.styleable.SSparkEditText)
        for (index in 0 until attributes.indexCount) {
            val attribute = attributes.getIndex(index)
            when (attribute) {
                R.styleable.SSparkEditText_ssparkFont -> {
