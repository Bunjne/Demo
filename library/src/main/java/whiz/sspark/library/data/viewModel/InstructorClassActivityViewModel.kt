...
import whiz.sspark.library.data.repository.InstructorClassActivityRepositoryImpl

class InstructorClassActivityViewModel(private val instructorClassActivityRepository: InstructorClassActivityRepositoryImpl) : ViewModel() {
    ...
    fun listPosts(classGroupId: String) {
        viewModelScope.launch {
            instructorClassActivityRepository.listPosts(classGroupId)
                .onStart {
    ...
    fun listOnlineClasses(classGroupId: String) {
        viewModelScope.launch {
            instructorClassActivityRepository.listOnlineClasses(classGroupId)
                .onStart {
                    _viewLoading.value = true
                }
                .onCompletion {
