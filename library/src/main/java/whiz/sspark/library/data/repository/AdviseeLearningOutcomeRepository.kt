...
import whiz.sspark.library.data.data_source.remote.service.AdviseeLearningOutcomeService
...
interface AdviseeLearningOutcomeRepository {
    suspend fun getLearningOutcome(termId: String, studentId: String): Flow<DataWrapperX<List<LearningOutcomeDTO>>>
}

class AdviseeLearningOutcomeRepositoryImpl(private val context: Context,
                                           private val remote: AdviseeLearningOutcomeService): AdviseeLearningOutcomeRepository, LearningOutcomeRepositoryImpl(context, remote) {
    override suspend fun getLearningOutcome(termId: String, studentId: String): Flow<DataWrapperX<List<LearningOutcomeDTO>>> {
        return flow {
            if (NetworkManager.isOnline(context)) {
                try {
                    val response = remote.getLearningOutcome(studentId, termId)
